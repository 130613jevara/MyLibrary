...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_book'])) {
    ...
    if (!empty($title) && !empty($author)) {
        $stmt = $conn->prepare("UPDATE books SET title = ?, author = ?, year = ?, genre = ?, description = ? WHERE id = ?");
        $stmt->execute([$title, $author, $year, $genre, $description, $id]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_book'])) {
    ...
    if (!empty($title) && !empty($author)) {
        $stmt = $conn->prepare("INSERT INTO books (title, author, year, genre, description, added_by) VALUES (?, ?, ?, ?, ?, ?)");
        $stmt->execute([$title, $author, $year, $genre, $description, $added_by]);
...
// Fetch books
if (!empty($search)) {
    $stmt = $conn->prepare("SELECT * FROM books WHERE title LIKE ? OR author LIKE ? ORDER BY date_added DESC");
    $searchTerm = "%$search%";
    $stmt->execute([$searchTerm, $searchTerm]);
} else {
    $stmt = $conn->query("SELECT * FROM books ORDER BY date_added DESC");
}
        ...
        <div class="add-book-section">
            ...
            <form method="POST" action="">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="author" name="author" required placeholder="e.g., Louisa May Alcott" value="<?= $editBook ? htmlspecialchars($editBook['author']) : '' ?>">
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="genre" name="genre" placeholder="e.g., Romance, Fantasy, Mystery" value="<?= $editBook ? htmlspecialchars($editBook['genre']) : '' ?>">
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="added_by" name="added_by" required placeholder="e.g., Sarah">
                </div>
                <?php endif; ?>
        ...
        <div class="books-grid">
            <?php foreach ($books as $book): ?>
                <div class="book-card">
                    ...
                    <?php endif; ?>
                    <div class="added-by">
                        💕 Added by <?= htmlspecialchars($book['added_by']) ?> 
